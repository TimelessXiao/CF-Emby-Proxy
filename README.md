...
推荐使用我们提供的交互式脚本来自动完成环境配置（包括 Cloudflare 认证、KV 创建、配置文件生成等）。
...
1. 安装 npm 依赖
2. 登录 Cloudflare
3. 创建所需的 KV Namespaces
4. 生成 `wrangler.json` 配置文件
5. 设置管理后台密钥 (ADMIN_TOKEN)

### 第五步：手动配置（如果跳过脚本）

如果你选择跳过上述脚本，请手动完成以下配置：
...
2. **准备配置文件**：
   ...
   - 填入你的 KV Namespace ID (`id` 和 `preview_id`)
   - 设置 Worker 名称
...
4. **修改 `worker.js`**，设置你的 Emby 服务器地址：
